...
abc = wd.find_elements(By.XPATH,'//*[@class="google-social-option"]')

for i in range(1,len(abc)):
	abc[i].click()

keyword="stock market recomendation"
f=open(keyword+".txt",'wb')

#key=wd.find_element_by_id('googlesearchinput') #
key=wd.find_element(By.ID, 'googlesearchinput')
key.send_keys(keyword) 

#button=wd.find_element_by_class_name('main-search') #
button=wd.find_element(By.CLASS_NAME, 'main-search')
button.click()

#get_social_media=wd.find_elements_by_xpath('//iframe[@class="cse-iframe"]') #
get_social_media=wd.find_elements(By.XPATH,'//iframe[@class="cse-iframe"]')
...
for i in links:
	...
	if "facebookcse" in i:
		...
		while(j<=5):
			...
			posts=wd.find_elements(By.XPATH,'//div[@class="gsc-webResult gsc-result"]/div/div/div/a')
			...
			page_numbers=wd.find_elements(By.XPATH,'//div[@class="gsc-cursor-page"]')
		...
		while(j<=5):
			...
			for each_link in links_for_post:
				...
				if hasXpath('//div[@class="js-tweet-text-container"]'):
					...
					content=wd1.find_elements(By.XPATH,'//div[@class="js-tweet-text-container"]')
					post_of_user=content.text
					arr_post.append(post_of_user)
			wd1.close()
			j=j+1
		...
		while(j<=5):
			...
			for each_link in links_for_post:
				...
				if hasXpath('//div[@class="C4VMK"]'):
					...
					content=wd1.find_element(By.XPATH,'//div[@class="C4VMK"]')
					post_of_user=content.text
					arr_post.append(post_of_user)
			wd1.close()
			j=j+1
		...
		while(j<=5):
			...
			for each_link in links_for_post:
				...
				if hasXpath('//button[@class="RCK Hsu mix Vxj aZc GmH adn a_A gpV hNT iyn BG7 NTm KhY"]'):
					...
					butt=wd1.find_element(By.XPATH,'//button[@class="RCK Hsu mix Vxj aZc GmH adn a_A gpV hNT iyn BG7 NTm KhY"]')
					butt.click()
